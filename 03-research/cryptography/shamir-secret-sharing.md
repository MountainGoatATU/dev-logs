#source #research  
2025-10-22
Internal links: [[2025-W6.1-meeting]]


---
## Documents
- Name: Shamir - How to Share a Secret
	- *Link*: [[Shamir-HowToShareASecret.pdf]]****
- Name: Shamir's Secret sharing for Authentication without reconstructing password
	- *Link*: [[Shamirs_Secret_Sharing_for_Authentication_without_Reconstructing_Password.pdf]] 
---
## Videos
- Name: WPW Sharmir Secret Sharing Tutorial
	- *Link*: https://www.youtube.com/watch?v=2UYescKYFxg
--- 
## Websites
- Name: SecretSharingDotNet
	- *Link*: https://github.com/shinji-san/SecretSharingDotNet (*We are going to use this in our .NET MAUI project*)
- Name: Shamir's secret sharing: Explanation and Visualisation
	- *Link*: https://evervault.com/blog/shamir-secret-sharing
- Name: Shamir's Secret Sharing Algorithm | Cryptography
	- *Link*: https://www.geeksforgeeks.org/computer-networks/shamirs-secret-sharing-algorithm-cryptography/
- Name: Mersenne Prime numbers
	- *Link*: https://www.mersenne.org/




